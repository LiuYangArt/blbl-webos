...
### 5. 后续补充：legacy 启动脚本不能重复注入

在后续继续修 Simulator 播放链路时，又暴露出一个和打包重写直接相关的新问题：

- 进入播放器后会听到两路声音同时播放
- 切到别的页面后，其中一路会停，但另一路仍像“后台播放器”一样继续播放

最终确认根因不是播放器业务层又创建了第二个 `<video>`，而是：

- `dist/index.html` 本身已经带有 Vite legacy 插件生成的：
  - `vite-legacy-polyfill`
  - `vite-legacy-entry`
- `scripts/prepare-webos.mjs` 后续又额外手工插入了一套 legacy 脚本
- 结果 `build/webos/index.html` 同时加载了两套前端入口
- React App 被启动了两次，播放器 effect 也跟着执行两次，于是出现“双实例 + 双音频”

这个问题的危险点在于：

1. 它表面上看像“播放器没清理干净”
2. 但真正根因其实在打包产物重写
3. 如果不先检查最终 HTML，而只在 `PlayerPage` 里继续补 `pause()` / `destroy()`，只会在错误层面反复打补丁

因此这轮补充修复的原则改成了：

- `prepare-webos` 只负责移除 modern module 入口
- 如果 `dist/index.html` 已经有 Vite 生成的 legacy 标记脚本，就直接保留
- 如果缺失，再补一套带固定 `id` 的 legacy 脚本
- 同时显式清理历史上那种“没有 `id` 的手工 legacy script”，让重写逻辑保持幂等

这轮修复后，Simulator 中“同一条视频双音频同时播放”的现象已消失。
...
7. 任何对 `build/webos/index.html` 的重写，都必须验证最终 HTML 里前端入口只会启动一次；“能启动”不等于“只启动了一次”。
8. 如果出现“双音频 / 双 telemetry / 同一 effect 重复执行”的现象，先检查最终产物里是否重复注入了脚本，而不是先假设业务组件泄漏。
