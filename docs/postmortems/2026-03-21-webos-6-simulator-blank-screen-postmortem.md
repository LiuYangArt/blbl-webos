...
本次 “Simulator 能启动应用，但只看到背景渐变、React UI 整体不显示” 的核心根因，是 **当前前端构建产物默认面向较新的浏览器，而 `webOS TV 6.0 / LG C1` 所在代际的 Web 引擎明显更旧**。
...
5. 这些语法错误发生在 React 挂载之前，所以最终现象就是：背景样式能显示，但整个 App UI 白屏。

修复方式是把构建目标显式降到 `chrome79`。修复后，`webOS 6.0 Simulator` 已能够正常显示应用 UI。
...
### 3. 重启 6.0 Simulator，确认重新加载的是新产物
