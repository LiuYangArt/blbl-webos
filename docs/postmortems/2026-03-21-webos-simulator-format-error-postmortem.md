...
这篇 postmortem 记录的是**当时那一版基线**下，“真机 UI 已恢复但视频仍然播不出来”的结论。

后续在继续清理旧代理残留、补环境识别、让 Simulator 针对 `browser-dev + Web0S/WebAppManager UA` 优先走兼容 MP4、并修掉 legacy 脚本重复注入导致的双实例问题之后，基线已经进一步变化。

最新新增事实是：

1. 用户已经在电视上做过一轮快速冒烟验证
2. **当前最新基线在电视上可以正常播放**

因此现在正确的理解方式应该是：

- 这篇文档保留的是“此前失败阶段”的复盘结论
- 它不能再被直接解读成“当前主分支真机一定仍然无法播放”
- 当前状态已经前进到：
  - Simulator 可播
  - 电视快速冒烟可播

但这不等于后续验证已经全部完成。当前仍需要继续验证：

1. 多样本是否稳定
2. 长时间播放是否稳定
3. 返回、暂停继续、切页面后是否有后台残留播放
4. 不同编码 / 不同内容类型是否仍存在回归
