...
- 新增本机外部媒体网关脚本：
  - `scripts/media-gateway.mjs`
  - 已支持两类入口：
    - `/media?url=...`
    - `/play-source?bvid=...&cid=...&quality=...&candidateIndex=...`
  - 其中 `/play-source` 已作为当前推荐入口：
    - 网关自己向 B 站请求 `playurl`
    - 自己选择兼容流地址
    - 自己补 `Referer / User-Agent / Range`
    - TV 端不再只会拿原始媒体直链去拼代理
- 新增模拟器代理验证脚本：
  - `scripts/verify-simulator-proxy.mjs`
  - `npm run verify:simulator-proxy`
- 播放器已支持读取：
  - 外部媒体代理 / 媒体网关地址模板
  - 启动即进入播放器的调试参数
  - 可选 debug telemetry 回传
- 播放器执行层已升级为更贴近官方推荐路径的结构：
  - `source[type]`
  - `MIME` 明确化
  - 真机预留 `mediaOption` 挂接位
- 播放器传输层已升级为结构化播放源请求：
  - 当前优先用 `bvid / cid / quality / candidateIndex` 访问外部媒体网关
  - 只有老式 `url` 代理配置时，才退回 `/media?url=...`
- 模拟器验证时，已通过“外部媒体网关 + 启动即进播放器”的方式确认：
  - 收到 `loadedmetadata`
  - 收到 `play`

这说明：

- 在模拟器环境中，只要不再直接播放 B 站原始媒体地址，而是改走可控媒体网关，当前 webOS 播放链路已经出现了**实质性改善**
- 当前推荐路线不是纯理论推导，已经有一轮仓库内验证结果支撑

需要注意：

- 这一轮还只是 **本机开发态的最小验证**
- 它证明了“媒体请求链路是关键问题”
- 还不等于“生产级外部媒体网关方案已经完成”
...
1. 把当前结构化 `/play-source` 接口继续扩展为更正式的“播放源解析接口”，逐步收敛前端对 B 站 `playurl` 细节的感知。
2. 让外部媒体网关输出更稳定的生产态描述：
   - 可直接消费的 `MP4` 地址
   - 或后续可切换的 `HLS` 地址
   - 以及必要的播放元信息
3. TV 端播放器继续保持：
...
4. 在真机上验证：
   - `source[type + mediaOption]`
   - 续播点是否优先走 `mediaOption`
   - 结构化媒体网关地址在真实网络下是否稳定
5. 再决定是否把“播放源解析”彻底下沉到网关侧，避免前端继续自己做多档位兼容流规划。
